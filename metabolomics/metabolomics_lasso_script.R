...
library(foreign)
#urlPackage <- "https://cran.r-project.org/src/contrib/Archive/foreign/foreign_0.8-52.tar.gz"
...
mergeytest_colsd <- apply(mergeytest[complete.cases(mergeytest),grep("HI|C8|C1", names(mergeytest), invert = F)], 2, sd, na.rm=T)
mergey_colsd <- apply(mergey[complete.cases(mergey),grep("HI|C8|C1", names(mergey), invert = F)], 2, sd, na.rm=T)
...
# rename the column names with cn
cn <- colnames(mergey)
cn
...
varlist_nocolin <- c(varlist[which(varlist %in% colnames(traindf))],
                     "sex", 
                     "race", 
                     "Antibiotics")
...
plot_varimp2 <- ggplot2::ggplot(df2) +
  ...
  theme(
    ...
    axis.text.y = element_text(
      color = "black",
      size = length(unique(df2$variable))*.7 ,
      angle = 0
    ),
...
metabolomiccs_glmer_scores <- tibble::rownames_to_column(metatranscriptomics_glmer_scores, "External_ID")
write.table(metabolomics_glmer_scores, "metabolomics_glmer_scores.txt", sep="\t", col.names=T, row.names=F, quote=F)

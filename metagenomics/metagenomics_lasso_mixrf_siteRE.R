...
# subset to only European ancestry
testing_metadata <- subset(testing_metadata, race == "White")
training_metadata <- subset(training_metadata, race == "White")
...
cn[is.na(cn)] <- c("Participant_ID", "site_name", "diagnosis", "consent_age", "sex", "Antibiotics")
head(cn,20)
tail(cn,20)
...
# stop()
numvariables <- c()
# lambdavec <- seq(from = 87, to = 107, by = 1)
...
# lassoFeatures <- unique(c(lassoFeatures, "Participant_ID", "site_name", "diagnosis", "consent_age", "sex", "race", "Antibiotics"))
lassoFeatures <- unique(c(lassoFeatures, "Participant_ID", "site_name", "diagnosis", "consent_age", "sex", "Antibiotics"))
...
# mymod_ONLYcovar <- lme4::glmer(as.formula("diagnosis ~ consent_age + sex + race + Antibiotics + site_name + (1|Participant_ID)"), 
#                                data = df_bestglm, 
#                                family = binomial)
mymod_ONLYcovar <- lme4::glmer(as.formula("diagnosis ~ consent_age + sex + Antibiotics + (1|Participant_ID) + (1|site_name)"), 
                               data = df_bestglm, 
                               family = binomial)
...
# find the average score for each individual
# m_pred_df <- merge(pred_df,predictionDF[,c("Participant_ID","diagnosis", "site_name", "consent_age", "sex", "race", "Antibiotics")], by="row.names")
m_pred_df <- merge(pred_df,predictionDF[,c("Participant_ID","diagnosis", "site_name", "consent_age", "sex", "Antibiotics")], by="row.names")
...
m_pred_df <- merge(null_model_predictions,predictionDF[,c("Participant_ID","diagnosis", "site_name", "consent_age", "sex", "Antibiotics")], by="row.names")
rownames(m_pred_df) <- m_pred_df$Row.names

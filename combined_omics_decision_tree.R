...
plot_model(loomod, vline.color = 'gray') + theme_bw() + ggtitle("")
ggsave(filename = "LOO_MGN.png", width=3, height=2.2, units="in", dpi=320)
loomod <- glm(as.formula(paste0("diagnosis ~ VRM + MGN + MBL + consent_age + sex")), data = df_for_model, family = "binomial")
plot_model(loomod, vline.color = 'gray') + theme_bw() + ggtitle("")
ggsave(filename = "LOO_MTS.png", width=3, height=2.2, units="in", dpi=320)
loomod <- glm(as.formula(paste0("diagnosis ~ MGN + MTS + MBL + consent_age + sex")), data = df_for_model, family = "binomial")
plot_model(loomod, vline.color = 'gray') + theme_bw() + ggtitle("")
ggsave(filename = "LOO_VRM.png", width=3, height=2.2, units="in", dpi=320)
loomod <- glm(as.formula(paste0("diagnosis ~ VRM + MGN + MTS + consent_age + sex")), data = df_for_model, family = "binomial")
plot_model(loomod, vline.color = 'gray') + theme_bw() + ggtitle("")
ggsave(filename = "LOO_MBL.png", width=3, height=2.2, units="in", dpi=320)
